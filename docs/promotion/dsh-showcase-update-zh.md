...
- 扫描**当前运行的工作区**，自动识别 6 种语言（JS/TS / Python / Go / Java / Rust / C-C++）、框架、入口文件、CI 配置、模块布局
- 生成语义化的架构报告（项目定位、架构风格、分层、组件职责、关系、运行流程、关键文件、阅读顺序）。可用当前 DSH LLM 时走 LLM 增强路径，否则自动降级为本地确定性分析
...
- 在新 Session 开始时自动注入精简的上下文摘要，让对话能延续已积累的项目知识（高价值决策、活跃 TODO、最近活动）
- **默认双通道检索设计**：零配置 BM25 关键词检索；可选 Embedding 端点启用双路召回（关键词 + 向量），用 Reciprocal Rank Fusion（k=60）融合 —— 双路都命中时融合分高，任一路缺失时优雅退化为加权打分
- Session 结束自动从当前 DSH LLM 抽取最多 4 条稳定语义记忆，含隐私清洗、长度限制、去重、失败兜底（LLM 不可用不崩）
- 交互式 Dashboard（5 个 Tab：概览 / 架构 / 任务动态 / 项目记忆 / 设置 / Git 历史）支持重扫、状态查询、整理记忆、查看工作活动
...
适合 DSH 中文圈主导场景，**与国际用户触达会减少**。

### 方式 B：英文 + 中文双语版（推荐）

英文版后追加中文 section。覆盖国际 + 中文，触达最优。
...
## ⚠️ 截图 URL 锁 v1.3.0 tag（不是 commit SHA）

历史版本各自指向当时 tag 的 commit；现在发 v1.4 时主帖不动，新版本自动指向 v1.3.0 截图（如要更新需改 discussion body）。